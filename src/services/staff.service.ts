...
async updateStaff (staffId: string, StaffData: IStaff, auth: payload) {
  if (auth.id !== staffId) {
    throw new Error("Access denied")
  }
...
async softDeleteStaff (staffId: string, auth: payload) {
  ...
  const deletedStaff = Staff.findByIdAndUpdate(
    staffId,
    { isDeleted: true },
    { new: true }
  );
...
async restoreStaff (staffId: string, auth: payload) {
  ...
  const restoredStaff = Staff.findByIdAndUpdate(
    staffId,
    { isDeleted: false },
    { new: true }
  );
...
async hardDeleteStaff (staffId: string, auth: payload) {
  if (auth.role !== "admin") {
    throw new Error("Access denied")
  }
